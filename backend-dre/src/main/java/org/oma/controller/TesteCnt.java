...
import org.oma.model.*;
import org.oma.service.*;
...
//    @Produces(MediaType.APPLICATION_JSON)
    ...
    //RECUPERATION TOKEN KEYCLOAK
    @POST
    @Path("/key")
    @PermitAll
    public Response getToken()  {
        try {
            // Votre logique d'authentification LDAP ici
            //...

            // Après l'authentification LDAP réussie, obtenez un token Keycloak
            String clientId = "votre_client_id";
            String clientSecret = "votre_client_secret";
            String realmName = "votre_realm_name";
            String grantType = "password";
            String usernameKeycloak = "username"; // Le nom d'utilisateur sous forme de chaîne de caractères
            String passwordKeycloak = "password"; // Le mot de passe sous forme de chaîne de caractères

            String authString = clientId + ":" + clientSecret;
            byte[] encodedAuth = Base64.getEncoder().encode(authString.getBytes(StandardCharsets.UTF_8));
            String authHeader = "Basic " + new String(encodedAuth);

            URI keycloakUri = new URI("http://localhost:8080/auth/realms/" + realmName + "/protocol/openid-connect/token"); // Ajustez l'URI selon votre configuration Keycloak
            ...
            String requestBody = "grant_type=" + grantType + "&client_id=" + clientId + "&username=" + usernameKeycloak + "&password=" + passwordKeycloak;
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                // Parsez la réponse pour extraire le token d'accès
                //...
            } else {
                throw new Exception("Échec de l'obtention du token Keycloak");
            }

            // Retournez le token d'accès ou une autre réponse appropriée
            //...

        } catch (Exception e) {
            return Response.status(Response.Status.UNAUTHORIZED).entity(new AuthResult(false, e.getMessage())).build();
        }
        return null;
    }
    //
...
//    @Inject
//    private OIDCRequestResolver requestResolver;
//
//    public String generateKeycloakToken(String username, String password) {
//        // Obtenir le résolveur de requête OIDC
//        OIDCRequestResolver requestResolver = this.requestResolver.createOIDCRequestResolver();
//
//        // Créer une requête OIDC
//        OIDCRequest request = requestResolver.createOIDCRequest();
//        request.setUsername(username);
//        request.setPassword(password);
//
//        // Générer le token Keycloak
//        AccessTokenResponse accessTokenResponse = requestResolver.createAccessToken(request);
//        String accessToken = accessTokenResponse.getIdToken();
//
//        return accessToken;
//    }
